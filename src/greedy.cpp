...
void Greedy::run() {
    ...
    // Track cumulative distance for fair comparison with CBBA/CBGA
    ...
    while (!available_tasks.empty()) { // Until you have assigned all tasks robot is capable of
        ...
        double best_task_distance = 0.0; // Track distance to best task
        ...
        for (int task_id : available_tasks) { // Find task that gives highest score
            ...
            // Calculate distance for cumulative tracking
            std::pair<int, int> task_position = world.getTaskLocation(task_id);
            double distance_to_task = Distance::getEuclideanDistance(
                current_pose.x, current_pose.y,
                task_position.first, task_position.second
            );
            double score = getTaskScore(task_id, cumulative_distance + distance_to_task);
            ...
            if (score > max_score) { // Save max score and id of robot that achieved it
                ...
                best_task_distance = distance_to_task;
            }
        ...
        if (best_task_id != -1) {
            ...
            cumulative_distance += best_task_distance;
    ...
    std::string robot_assignment = "Robot " + std::to_string(robot.getID()) + " greedy assignment: ";
    for (int task_id : assigned_task_order) {
        robot_assignment += std::to_string(task_id) + " ";
    }
    robot.log_info(robot_assignment);
...
double Greedy::getTaskScore(int task_id, double total_distance) {
    ...
    // Apply SAME discount formula as CBBA with cumulative distance
    double score = reward * pow(discount_factor, total_distance);
