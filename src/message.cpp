...
/*void Message::updateRobotMessageQueue(Robot& receiver) {
    std::vector<Msg> message_queue = receiver.getMessageQueue();
    message_queue.push_back(msg);
}

/*void Message::receiveMessages(World& world, Robot& receiver) {
    std::lock_guard<std::mutex> lock(world.getWorldMutex());
    std::unordered_map<int,std::vector<Msg>> world_msg_tracker = world.getMessageTracker();
    ...
    if (world_msg_tracker.find(receiverID) != world_msg_tracker.end()) {
        ...
        if (!messages.empty()) {
            Msg msg = messages.back();  // Assuming we take the last message
            // Add message to receiver robot message queue
            updateRobotMessageQueue(receiver);
            std::cout << "Robot " << receiver.getID() << " received a message from Robot " << msg.id << std::endl;
        }

        /*Msg msg = world_msg_tracker[receiverID];
        // Add message to receiver robot message queue
        updateRobotMessageQueue(receiver);
        std::cout << "Robot " << receiver.getID() << " received a message from Robot " << msg.id << std::endl;

        // Should this function be in robot because of where it is called in behavior_tree.cpp not having access to the message instance directly? I think no
    }
